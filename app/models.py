...
class User(UserMixin, db.Model):
    ...
    # Wishes this user has reserved on OTHER people's profiles
    reservations = db.relationship(
        "WishReservation",
        foreign_keys="WishReservation.reserved_by_id",
        backref="reserver",
        lazy=True,
        cascade="all, delete-orphan",
    )
...
class WishItem(db.Model):
    ...
    # One-to-one: a wish can have at most one active reservation
    reservation = db.relationship(
        "WishReservation",
        backref="wish",
        uselist=False,
        cascade="all, delete-orphan",
        foreign_keys="WishReservation.wish_id",
    )


class WishReservation(db.Model):
    __tablename__ = "wish_reservations"

    id             = db.Column(db.Integer, primary_key=True)
    wish_id        = db.Column(
        db.Integer,
        db.ForeignKey("wish_items.id", ondelete="CASCADE"),
        nullable=False,
        unique=True,   # one reservation per wish
    )
    ...
    reserved_at = db.Column(db.DateTime, server_default=db.func.now())
